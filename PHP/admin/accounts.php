...
// SQL where clause
$where = '';
$where .= $search ? 'WHERE (email LIKE :search OR displayName LIKE :search) ' : '';
// Retrieve the total number of products
$stmt = $pdo->prepare('SELECT COUNT(*) AS total FROM account ' . $where);
if ($search) $stmt->bindParam('search', $param3, PDO::PARAM_STR);
$stmt->execute();
$accounts_total = $stmt->fetchColumn();
// SQL query to get all products from the "products" table
$stmt = $pdo->prepare('SELECT * FROM account ' . $where . ' ORDER BY ' . $order_by . ' ' . $order . ' LIMIT :start_results,:num_results');
// Bind params
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?=htmlspecialchars($account['user'], ENT_QUOTES)?></td>
                    <td><?=htmlspecialchars($account['displayName'], ENT_QUOTES)?></td>
                    <td class="responsive-hidden"><?=$account['auth']?></td>
